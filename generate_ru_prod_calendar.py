...
def generate_calendar(output_file: Path) -> None:
    data = load_data()
    ...
    for year, year_data in sorted(data.items()):
        ...
        non_working_days = year_data.get("non_working_days", {})
        working_weekends = {
            transfer["from"]
            for transfer in year_data.get("transfers", [])
            if transfer.get("from") not in non_working_days
        }

        events: dict[str, str] = {}
        for day in year_days(year_number):
            day_iso = iso(day)
            if is_weekend(day) and day_iso not in working_weekends:
                events[day_iso] = "Выходной"

        for day, name in non_working_days.items():
            events[day] = event_title("non_working", name)

        for day, title in sorted(events.items()):
